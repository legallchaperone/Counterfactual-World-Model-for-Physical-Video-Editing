...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    ...
    parser.add_argument("--add-planner-adapter", type=Path, required=True, help="add-planner LoRA adapter for the add stage (e2w_add)")
...
def run_pipeline(args: argparse.Namespace) -> dict[str, Any]:
    ...
    add_cmd = [
        ...
        str(add_stage),
        "--planner-adapter",
        str(args.add_planner_adapter.resolve()),
        "--vace-sample-steps",
    ...
    remove_cmd = [
        str(args.python.resolve()),
        str((TOOLS / "e2w_remove.py").resolve()),
        "--eval-jsonl",
        str(remove_eval),
        "--output-dir",
        str(remove_stage),
